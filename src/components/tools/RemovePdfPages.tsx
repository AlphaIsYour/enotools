...
export default function RemovePdfPages() {
  ...
  const parsePages = (input: string, max: number): Set<number> | null => {
  ...
  const removePages = async () => {
    ...
    const pagesToRemove = parsePages(removeInput, pageCount);
    if (!pagesToRemove) {
      setError(`Invalid page numbers. Use numbers 1-${pageCount}, e.g., "1,3,5-7".`);
      return;
    }
    ...
    try {
      ...
      link.download = fileName.replace(/\.pdf$/i, "_pages_removed.pdf");
      link.href = url;
      link.click();
    ...
    <div className="space-y-6">
        ...
        <div className="rounded-lg border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-6">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-surface-500 dark:text-surface-400">
              Enter comma-separated page numbers or ranges to remove (1-{pageCount}).
            </p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-lg border border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 p-6">
        ...
        <p className="text-xs text-surface-600 dark:text-surface-400">
          Upload a PDF and specify which pages to remove. The remaining pages will be saved as a new
          PDF. You cannot remove all pages.
        </p>
      </div>
    </div>
  );
